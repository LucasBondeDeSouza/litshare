    ...
    return (
        input.length > 0 && datas.length > 0 && (
            <div className="list-group position-absolute w-100">
                {datas.map((data) => (
                    <div key={data.id} onClick={() => handleClick(data.social_handle, data.title, data.key.replace('/works/', ''))} className="list-group-line d-flex justify-content-between align-items-center p-2">
                        {data.title ? (
